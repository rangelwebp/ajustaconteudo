	...
	return (
		<>
			...
			<button
				...
				aria-label={isFloating ? "Minimizar painel" : "Abrir painel flutuante"}
					...
					<div className="max-w-2xl mx-auto">
						...
						<header className="mb-10">
							...
							<p className="mt-1.5 text-sm text-[#9C917A]">
								Prepare um texto para publicação em diferentes formatos.
							</p>
	...
	return (
		<div className="bg-[#1B1712] border border-[#35301F] rounded-xl shadow-2xl shadow-black/50 overflow-hidden">
			<div className="px-4 py-3 border-b border-[#35301F] flex justify-between items-center">
				<span className="font-serif text-[#EDE6D6]">AjustaConteúdo</span>
				...
				<div className="p-4 space-y-4">
					...
					{formatted && (
						<div className="pt-3 border-t border-[#35301F]">
							<ResultSheet formatted={formatted} onCopy={onCopy} copiedField={undefined} compact />
						</div>
					)}
				</div>
			)}
		...
		<div className="space-y-8">
			...
			{formatted && (
				<div>
					...
					<ResultSheet formatted={formatted} onCopy={onCopy} copiedField={copiedField} />
				</div>
			)}
		</div>
	);
...
function InputField({ label, name, value, onChange, placeholder, dot, required }: any) {
	return (
		<div>
...
function TextAreaField({ label, name, value, onChange, placeholder, rows, dot, required }: any) {
	return (
		<div>
			<label className="flex items-center gap-2 text-sm text-[#9C917A] mb-1.5">
				<span
...
function ResultSheet({ formatted, onCopy, copiedField, compact }: any) {
	...
	return (
		<div className={compact ? "space-y-3" : "bg-[#1B1712] border border-[#35301F] rounded-xl divide-y divide-[#2A251C]"}>
			{rows.map((row) => (
				<div key={row.key} className={compact ? "" : "px-5 py-4"}>
					<div className="flex justify-between items-center mb-1.5">
						...
						<button
							onClick={() => onCopy(row.value, row.label, row.key)}
...
function ImportExportPanel({ history, exportHistory, importHistory, clearHistory, onClose }: any) {
	return (
...
function HistoryPanel({ history, loadFromHistory, deleteFromHistory, formatDate, onClose }: any) {
	return (
		<div className="bg-[#1B1712] border border-[#35301F] rounded-xl p-5 mb-6">
			<div className="flex justify-between items-center mb-4">
				<h2 className="text-[#EDE6D6]">
					Histórico ({history.length} {history.length === 1 ? "item" : "itens"})
				</h2>
				<button
					onClick={onClose}
					className="text-[#9C917A] hover:text-[#EDE6D6] p-1 transition-colors">
					...
					history.map((item: HistoryItem) => (
						<div
							...
							<div className="min-w-0">
								<p className="text-sm text-[#EDE6D6] truncate">{item.titulo}</p>
								<p className="text-xs text-[#6B6350] mt-0.5">{formatDate(item.createdAt)}</p>
							</div>
							<button
